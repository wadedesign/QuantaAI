...
from utils.WF2.WadderGameUP import DiscordColor, DEFAULT_COLOR
...
class Tictactoe(commands.Cog):
    ...
    def __init__(self, cross: nextcord.User, circle: nextcord.User) -> None:
        self.cross = cross
        self.circle = circle

        self.board: list[list[str]] = [[self.BLANK for _ in range(3)] for _ in range(3)]
        self.turn: nextcord.User = self.cross

        self.winner: Optional[nextcord.User] = None
        ...
        self.emoji_to_player: dict[nextcord.User, str] = {
            self.CIRCLE: self.circle,
            self.CROSS: self.cross,
        }
        self.player_to_emoji: dict[str, nextcord.User] = {
    ...
    def make_embed(self, *, game_over: bool = False) -> nextcord.Embed:
        embed = nextcord.Embed(color=self.embed_color)
        if game_over:
    ...
    def make_move(self, emoji: str, user: nextcord.User) -> list:
...
class TicTacToeCommands(commands.Cog):
    ...
    @commands.command()
    async def tictactoe(self, ctx, player1: nextcord.User, player2: nextcord.User = None):
        if player2 is None:
            player2 = self.bot.user  # Set the bot as the second player
