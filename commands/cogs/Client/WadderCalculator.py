...
class Calculator(commands.Cog):
    ...
    async def calc(self, interaction: nextcord.Interaction, *, formula):
        """Evaluate math expressions."""
        try:
            answer = self.nsp.eval(formula)
        except Exception as e:
            return await interaction.send(f"Could not parse formula. {e}")
        ...
        # Send answer
        await interaction.send(f"{formula} = {answer}")
