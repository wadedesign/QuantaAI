...
from WF0.classes import Config, CustomEmojis, NoneClass

from WF0.errors import BlackListed, print_error
from WF0.functions import load_json, read_file
...
async def get_prefix(_bot, message):
    ...
    ----------
        ...
        message (nextcord.Message): the message to get some metadata
...
class WMBot(commands.Bot):
    ...
    async def get_context(self, message: nextcord.Message, *, cls: commands.Context = None) -> commands.Context:
    ...
    async def fetch_banner(self, user: nextcord.User) -> str:
        # TODO: Cache
    ...
    @property
    def members(self) -> Generator[nextcord.Member, None, None]:
    ...
    @property
    def humans(self) -> Generator[nextcord.User, None, None]:
        """Use this to get all the members of the bot"""
    ...
    @property
    def bots(self) -> Generator[nextcord.User, None, None]:
        """Use this to get all the members of the bot"""
        for user in self.users:
            if user.bot:
                yield user
    ...
    def get_user_named(self, name: str) -> Union[nextcord.User, None]:
...
class WMBotContext(commands.Context):
    ...
    async def send(self, *args, **kwargs) -> nextcord.Message:
        ...
        ----------
        *args : tuple
            Arguments to be passed to nextcord.abc.Messagable.send or nextcord.Message.reply
        **kwargs : dict, optional
            Keyword Arguments to be passed to nextcord.abc.Messagable.send or nextcord.Message.reply
        ...
        nextcord.InvalidArgument
            ...
            or the reference object is not a nextcord.Message,
            nextcord.MessageReference or nextcord.PartialMessage.
        ...
        try:
            ...
            if error.status == 400 and error.code == 50035:
                ...
                message = await self.send(
                    embed=nextcord.Embed(title="Content too long", description=f"Uploaded to cloud: {url}")
                )
            else:
                raise error
