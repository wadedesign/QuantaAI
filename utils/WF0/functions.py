...
def button_from_json(json_obj: dict, *, cls: Any = nextcord.ui.Button) -> nextcord.Button:
...
def find_user_named(users: Iterable[nextcord.User], name: str) -> nextcord.User:
    result = None
    ...
    if len(name) > 5 and name[-5] == "#":
        ...
        result = nextcord.utils.get(users, name=name[:-5], discriminator=potential_discriminator)
    ...
    return nextcord.utils.find(pred, users)
...
async def get_agreement(
    ...
    text: str,
    destination: nextcord.abc.Messageable = None,
    target: nextcord.User = None,
    timeout: int = 20,
) -> Union[bool, None]:
...
def get_flag(flag: str) -> str:
    ...
    ----------
    flag : str
        The flag name, nextcord.User.public_flags is a good way to get all the flags
    ...
    if flag == "hypesquad_brilliance":
        ...
        return f"{get_custom_emoji('badges.nextcord_partner')} | Discord Partner"
    elif flag == "staff":
...
async def get_image(
    ...
    url : typing.Union[ nextcord.Member, nextcord.Emoji, nextcord.PartialEmoji, None, str ], optional
    ...
    if interaction.message.reference:
        ...
        if ref.embeds:
            if ref.embeds[0].image.url != nextcord.Embed.Empty:
                if is_image(ref.embeds[0].image.url):
                    return ref.embeds[0].image.url

            if ref.embeds[0].thumbnail.url != nextcord.Embed.Empty:
    ...
    if interaction.message.attachments:
        ...
        item = interaction.message.attachments[0].url or interaction.message.attachments[0].proxy_url

        if is_image(item):
            return interaction.message.attachments[0].proxy_url or interaction.message.attachments[0].url
...
def get_random_color() -> nextcord.Color:
    """Returns a random color from nextcord.Color
    ...
    return nextcord.Color(random.randint(0, 0xFFFFFF))
...
def make_permissions(perm_value: Union[int, str], *, oauth_url: int = None) -> nextcord.Permissions:
    """Makes a nextcord.Permissions object from the given value
    ...
    'https://nextcord.com/oauth2/authorize?client_id=723234115746398219&scope=bot&permissions=8'
    >>> make_permissions('none', oauth_url=723234115746398219)
    'https://nextcord.com/oauth2/authorize?client_id=723234115746398219&scope=bot&permissions=0'
    >>> make_permissions('all', oauth_url=723234115746398219)
    'https://nextcord.com/oauth2/authorize?client_id=723234115746398219&scope=bot&permissions=68719476735'
    ...
    """
    perm = (
        nextcord.Permissions(perm_value) if isinstance(perm_value, int) else getattr(nextcord.Permissions, perm_value)()
    )
    return nextcord.utils.oauth_url(oauth_url, permissions=perm) if oauth_url else perm
