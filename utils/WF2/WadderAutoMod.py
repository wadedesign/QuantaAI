...
import emojis
from utils.WF2.WadderConfigs import DEFAULT_BANNED_WORDS, EMOJIS, RED_COLOR, DEFAULT_AUTOMOD_CONFIG
from datetime import datetime
from re import search
from collections import Counter

from utils.WF2.WadderEmbeder import error_embed, success_embed
from utils.WF2.WadderUI import BasicView, Paginator, SelectWithMultipleOptions
from utils.WF2.WadderExceptions import AutomodModuleNotEnabled
...
class Automod(commands.Cog):
    def __init__(self, client):
        ...
        self.invite_regex = re.compile(r'((http(s|):\/\/|)(nextcord)(\.(gg|io|me)\/|app\.com\/invite\/)([0-z]+))')
        self.zalgo_regex = re.compile(r"%CC%", re.MULTILINE)

    def mod_perms(self, m: nextcord.Message) -> bool:
    ...
    async def on_automod_trigger(self, am_config: dict, message: nextcord.Message, module: str):
    ...
    async def efficient_automod(self, msg: nextcord.Message):
    ...
    async def banned_words(self, msg: nextcord.Message, m: dict) -> bool:
    ...
    async def all_caps(self, msg: nextcord.Message, m: dict) -> bool:
    ...
    async def duplicate_text(self, msg: nextcord.Message, m: dict) -> bool:
    ...
    async def message_spam(self, msg: nextcord.Message, m: dict) -> bool:
        def _check(m):
    ...
    async def invites(self, msg: nextcord.Message, m: dict) -> bool:
    ...
    async def links(self, msg: nextcord.Message, m: dict) -> bool:
    ...
    async def mass_mentions(self, msg: nextcord.Message, m: dict) -> bool:
    ...
    async def emoji_spam(self, msg: nextcord.Message, m: dict) -> bool:
    ...
    async def zalgo_text(self, msg: nextcord.Message, m: dict) -> bool:
    ...
    async def ghostping_delete(self, msgobj):
        ...
        if mentions or msgobj.mention_everyone:
            ...
            ).set_thumbnail(url="https://cdn.nextcordapp.com/emojis/527884882010177536.png?v=1"
...
async def show_automod_config(ctx: commands.Context) -> t.Tuple[nextcord.Embed, nextcord.ui.View]:
...
async def view_badword_list(ctx: commands.Context) -> t.Tuple[nextcord.Embed, t.Optional[nextcord.ui.View]]:
    g = await ctx.bot.get_guild_config(ctx.guild.id)
    am = g['automod']
    enabled = True if am['banned_words']['enabled'] else False
    if not enabled:
...
async def am_whitelist_func(ctx: commands.Context, choice: bool, setting: t.Optional[t.Union[nextcord.Role, nextcord.TextChannel]]) -> bool:
    g = await ctx.bot.get_guild_config(ctx.guild.id)
    am = g['automod']
    if choice:
        if isinstance(setting, nextcord.TextChannel):
...
async def view_whitelisted_links_list(ctx: commands.Context) -> t.Tuple[nextcord.Embed, nextcord.ui.View]:
    g = await ctx.bot.get_guild_config(ctx.guild.id)
    am = g['automod']
    enabled = True if am['links']['enabled'] else False
    if not enabled:
...
async def am_enable_module_dropdown(ctx: commands.Context) -> t.Tuple[nextcord.Embed, nextcord.ui.View]:
    g = await ctx.bot.get_guild_config(ctx.guild.id)
    am = g['automod']
    ...
    button = nextcord.ui.Button(style=nextcord.ButtonStyle.blurple, label="Continue")
    cancel_btn = nextcord.ui.Button(style=nextcord.ButtonStyle.danger, label="Cancel")
...
class AutomodConfigView(nextcord.ui.View):
    ...
    @nextcord.ui.button(label="Filters Config", style=nextcord.ButtonStyle.blurple, disabled=True)
    async def filter_show(self, b: nextcord.Button, i: nextcord.Interaction):
        for item in self.children:
            item.disabled = False
        b.disabled = True
        await i.message.edit(embed=self.embeds[0], view=self)

    @nextcord.ui.button(label="Whitelist Config", style=nextcord.ButtonStyle.green)
    async def whitelist_show(self, b: nextcord.Button, i: nextcord.Interaction):
        for item in self.children:
            item.disabled = False
        b.disabled = True
        await i.message.edit(embed=self.embeds[1], view=self)

    async def interaction_check(self, i: nextcord.Interaction):
